...
@section('content')
    ...
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1>5 Time Prayer | {{$ambil_trainee->name}}</h1>
          </div>
        </div>
      </div><!-- /.container-fluid -->
    </section>
          ...
          <div class="col-12">
            <div class="card">
              <div class="card-header">
                ...
                <form action="{{ route('fivetimeprayer-week', $ambil_trainee->nip) }}" method="POST">
                  @csrf
                  <div class="form-inline">
                    ...
                    <a href="{{ route('Fivetimeprayer-Asisten', $ambil_trainee->nip) }}" class="btn btn-danger ml-2">Reset</a>
                  </div>
                </form>
              </div>
              <!-- /.card-header -->
              ...
              <div class="card-body">
                <div class="table-responsive">
                  <table id="example2" class="table table-bordered table-hover">
                    ...
                    <tbody>
                    @if ($ambil_timeprayer->isNotEmpty())
                  @foreach($ambil_timeprayer as $data)
                  <tr>
                      <td class="col-1">{{ $data->created_at }}</td>
                      <td class="col-6">{{ $data->poin_prayer }}
                            ...
                            <div class="modal fade" id="ipoin-{{ $data->id }}" data-backdrop="static" data-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
                                <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable" role="document">
                                  <div class="modal-content">
                                    ...
                                    <form action="{{ route('fivetimeprayer-poin', ['id' => $data->id]) }}" method="POST">
                                      @csrf
                                      @method('PATCH')
                                      <div class="modal-body" style="max-height: 500px; overflow-y: auto;">
                                        <div class="form-group">
                                          <label for="Prayer topic">Prayer topic :</label>
                                          <textarea name="bible" disabled class="form-control">{{ $data->poin_prayer }}</textarea>
                                          <select name="poinprayer" class="form-control mt-1" style="background-color:#001F3F; color:#FFFf;">
